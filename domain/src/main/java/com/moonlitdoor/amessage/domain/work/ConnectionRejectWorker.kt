...
import com.moonlitdoor.amessage.dto.ConnectionRejectPayload
...
@HiltWorker
class ConnectionRejectWorker @AssistedInject constructor(
  ...
  override suspend fun doWork(): Result = coroutineScope {
    withContext(Dispatchers.IO) {
      inputData.getString(CONNECTION_UUID)?.let { connectionUuid ->
        ...
        return@withContext when (client.send(ConnectionRejectPayload(), ConnectionMapper.mapToDto(connectionEntity))) {
          NetworkRequestStatus.SENT -> Result.success(ConnectionDeleteWorker.data(Id(UUID.fromString(connectionUuid))))
  ...
  companion object {
    ...
    private fun request() = OneTimeWorkRequest.Builder(ConnectionRejectWorker::class.java)
    ...
    fun enqueue(workManager: WorkManager, connectionId: Id) = workManager
      .beginWith(request().setInputData(data(connectionId = connectionId)).build())
      .then(ConnectionDeleteWorker.request().build())
      .enqueue()
  }
}
