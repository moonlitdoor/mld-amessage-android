...
@Composable
fun Conversations(navHostController: NavHostController, viewModel: ConversationsViewModel, showBottomBar: (Boolean) -> Unit) {
  ...
  viewState.let { state ->
    Ensure exhaustive when (state) {
      ...
      is ConversationsViewState.Loading -> Loading(R.string.conversations_title)
